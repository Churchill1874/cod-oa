...
public class CustomerAuthAspect {
    ...
    @Pointcut("@annotation(com.ent.codoa.common.annotation.CustomerAuthCheck)")
    public void customerAuthPointCut(){

    }

    @Before("customerAuthPointCut()")
    public void beforeCut(JoinPoint joinPoint){
        LoginToken loginToken = TokenTools.getLoginToken(true);
        if (!loginToken.getCustomerMenu()){
            if("cn".equals(loginToken.getLang())){
                throw new DataException("客户管理操作权限受限,请联系管理员");
            }
            if("jp".equals(loginToken.getLang())){
                throw new DataException("顧客管理操作の権限が制限されています。管理者にご連絡ください。");
            }
