...
public class SalarySettlementAdd implements Serializable {
    ...
    private BigDecimal weekdayOvertimePayRate = BigDecimal.ZERO;;

    @ApiModelProperty("平日加班费")
    private BigDecimal weekdayOvertimeAmount = BigDecimal.ZERO;;

    @NotNull(message = "周末加班费用比例不能为空")
    @ApiModelProperty(value = "周末加班费比例 例如:1就是没有 1.1就是1.1倍的意思, 2就是两倍的意思",required = true)
    private BigDecimal weekendOvertimePayRate = BigDecimal.ZERO;
    ...
    @ApiModelProperty(value = "所得税交税比例",required = true)
    private Integer payTaxesRate = 0;

    @ApiModelProperty(value = "所得税交税金额",required = true)
    private BigDecimal payTaxesAmount = BigDecimal.ZERO;;

    @ApiModelProperty("住民税比例")
    private Integer residentTaxRate = 0;

    @ApiModelProperty("住民税金额")
    private BigDecimal residentTaxAmount = BigDecimal.ZERO;;

    @ApiModelProperty("估算工资")
    private BigDecimal estimateSalary = BigDecimal.ZERO;;
