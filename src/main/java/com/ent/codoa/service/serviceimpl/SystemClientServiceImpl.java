...
public class SystemClientServiceImpl extends ServiceImpl<SystemClientMapper, SystemClient> implements SystemClientService {
    ...
    @Override
    public IPage<SystemClient> queryPage(SystemClientPage po) {
        // 存入构造器 要分页的页号参数 和 每页显示几条参数
        // 注意:因为IPage是mybatisplus 用来自动给你分页转化的对象 它要求必须通过表相对的实体类的声明来识别分页的表 <实体类>
        IPage<SystemClient> iPage = new Page<>(po.getPageNum(), po.getPageSize());
        ...
        queryWrapper.lambda()
            ...
            .eq(StringUtils.isNotBlank(po.getAccount()), SystemClient::getAccount, po.getAccount())
            .eq(StringUtils.isNotBlank(po.getName()), SystemClient::getName, po.getName())
            .eq(po.getStatus() != null, SystemClient::getStatus, po.getStatus())
